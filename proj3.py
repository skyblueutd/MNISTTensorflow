...
layer_cnn0 = tf.layers.Conv2D(64, 5, padding="same", activation = tf.nn.relu)
...
layer_dropout = tf.layers.Dropout(rate=0.25)
...
def prediction(X, training):
    ...
    pool4x4 = layer_pool4x4(inputs) # this must be the first layer
    cnn0 = layer_cnn0(pool4x4)
    #second convolution layer
    # pool3x3 = layer_pool3x3(cnn0)
    # cnn1 = layer_cnn1(pool3x3)
    output = layer_pool2x2(cnn0)

    values = layer_flatten(output)
